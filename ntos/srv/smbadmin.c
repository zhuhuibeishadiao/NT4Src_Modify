...

{
    ...
    //
    /* 验证SMB格式是否正确 */
    if ( (connection->SmbDialect <= SmbDialectNtLanMan && request->WordCount != 13)    ||
         (connection->SmbDialect >  SmbDialectNtLanMan && request->WordCount != 10 )   ||
    ...
    //
    /* 如果这是一个LnaMan 2.1 或者更好的， 该会话的respone可能要比request长，分配一个额外的SMB Buffer。该Buffer在我们发送了回复后释放 */
    ...
    if ( connection->SmbDialect <= SmbDialectNtLanMan) 
    {
        ...
        caseSensitivePasswordLength = (CLONG)SmbGetUshort( &ntRequest->CaseSensitivePasswordLength );
        caseSensitivePassword = caseInsensitivePassword + caseInsensitivePasswordLength;
    ...
    else 
    {
        ...
        caseInsensitivePasswordLength   = (CLONG)SmbGetUshort( &request->PasswordLength );
