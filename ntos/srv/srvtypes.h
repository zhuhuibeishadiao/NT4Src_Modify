...
typedef struct _POOL_HEADER {
    ...
    //
    // 这是一个Vector的基址，vector的长度是LOOK_ASIDE_MAX_ELEMENTS。每一个元素是可能被释放的块
...
typedef struct _WORK_QUEUE {
    ...
    union {
        ...
        struct _QUEUE {
            ...
            //
            struct _RFCB      *CachedFreeRfcb;              // RfcbFreeList是一个RFCB结构的空闲链表，用来削减pool申请的数量
            ...
            //
            // This work item is queued on the Queue above to cause
            //  one worker thread to go away
            //
            SPECIAL_WORK_ITEM   KillOneThreadWorkItem;
