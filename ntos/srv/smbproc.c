...
Routine Description:
    ...
    该例程派发不同的SMB命令到合适的例程中。基于当前的Command code，它通过派发表SrvFspSmbDispatchTable间接调用。
    SMB 处理者执行该命令，更新SMB中的指针，同时返回一个指示，是否有另一个Command需要被处理。如果是，该例程派发下一个命令。
    如果没有，则该例程发送一个回应。亦或者，SMB的处理者可以发起一个异步处理，它可以指示这么做，那么该例程只是简单的返回。
    ...
    该例程最初的调用是由SrvRestartSmbReceived发起的，它是一个FSM例程，通TdiReceive中获得控制权。 它同样被调用从其他重启例程当异步操作时，比如文件读，完成，chained命令处理。
    ...
    SrvRestartSmbReceive 加载SMB指针，封装进work context块中。注意，它拷贝SMB中的第一个命令到WorkContext->NextCommand中。
    当一个 AndX命令被处理时，SMB处理者必须加载chained command到NextCommand，在调用该例程去处理命令时
Arguments:
    ...
    WorkContext --- 提供一个work context block指针，它包含要被处理的SMB信息。该块在被处理过程中会被更新
...

{
    ...
    /* 循环处理SMB，直到一个状态返回，而不是SmbStatusMoreCommands返回。当一个SMB处理者返回这个command 时，意味着它设置好了WorkContext->NextCommand。故，我们可以处理下一个命令 */
    while ( TRUE ) 
    {
        ...
        /* 如果返回的不是SmbStatusMoreCommands，则到此结束。调用SrvEndSmbProcessing来发送回应，同时销毁WorkContext */
        ...
        /* 在SMB中有更多的命令，验证SMB来确保它有一个有效的头，同时word cont和byte count在一个有效的范围内 */
...

{
    ...
    if ( !irp->Cancel &&
        ...
        IF_DEBUG(NETWORK_ERRORS) {
            SrvPrint2( "SrvRestartReceive: status = %X for IRP %08lx\n",
