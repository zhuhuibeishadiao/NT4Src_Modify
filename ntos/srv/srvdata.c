...
PWORK_QUEUE SrvWorkQueues = 0;    // first queue in the allocated memory 第一个queue
...
/* 服务器APIs的Dispatch Table。APIs根据传递给NtFsControlFile的控制码来派发调用 */
...

{
    ...
    /* 创建资源锁来序列化访问XACTSRV port。该资源所用来保护访问共享内存计数和共享内存堆 */
    ...
    /* 各种client的名字，这个数组对应SMB_DIALECT 枚举的所有类型 */
