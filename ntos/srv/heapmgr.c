...
SrvInterlockedAllocate( PLOOK_ASIDE_LIST l, ULONG NumberOfBytes, PLONG statistics )
{
    ...
    PPOOL_HEADER *pentry = NumberOfBytes > LOOK_ASIDE_SWITCHOVER ? l->LargeFreeList : l->SmallFreeList;
...
Routine Description:
    ...
    该例程在服务器端分配nonpaged pool。申请时会检查服务器端总共的nonpaged pool的数量是否超过了配置的限制大小
Arguments:
...

{
    ...
    //
    /* 以下类似于从lookaside list分配，一共有13个空闲的块可用来分配，每一个都是POOL_HEADER结构
    */
    if( SrvWorkQueues ) 
    {
