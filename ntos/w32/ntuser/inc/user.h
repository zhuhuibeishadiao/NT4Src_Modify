...
typedef struct tagWND {          // wnd
    ...
    struct tagWND *spwndNext;    // Handle to the next window                       // 下一个窗口
    struct tagWND *spwndParent;  // Backpointer to the parent window.               // 父窗口
    struct tagWND *spwndChild;   // Handle to child                                 // 子窗口
    struct tagWND *spwndOwner;   // Popup window owner field                        // popwindow 专用，owner窗口

    PVOID         pwo;          // WNDOBJ in gdi that is associated with window     // 当前窗口在gdi中wndobj 对象

    RECT          rcWindow;     // Window outer rectangle                           // 窗口外形
    RECT          rcClient;     // Client rectangle                                 // 客户区外形

    WNDPROC_PWND lpfnWndProc;   // HI BIT on means WOW PROC                         // 窗口消息处理函数

    PCLS          pcls;         // Pointer to window class                          // 窗口类
    int           cbwndExtra;   // Extra bytes in window                            // 窗口所带的扩展字节信息

    HRGN          hrgnUpdate;   // Accumulated paint region                         // 累计的paint 区域

    struct tagWND *spwndLastActive; // Last active in owner/ownee list              // 在owner list中上一个active窗口
    struct tagPROP *ppropList;  // Pointer to first property in list                // 指向链表中的第一个属性
    PSBINFO        pSBInfo;   // Words used for scrolling                           // 用于scroll的words

    struct tagMENU *spmenuSys;  // Handle to system menu                            // 指向系统菜单句柄
    struct tagMENU *spmenu;     // Menu handle or ID                                // 菜单handle or id
    LARGE_UNICODE_STRING strName;                                                   // 窗口名字？？

    /*
     * Fullscreen information
     */
    BYTE          bFullScreen;  // record full screen state info                    // 是否全屏
    BYTE          cDC;          // count of DCs associated with window              // 和当前窗口关联的DC数量
    WORD          fnid;         // record window proc used by this hwnd             // 通过GETFNID记录当前窗口使用proc情况
                                // access through GETFNID
    DWORD         dwExpWinVer;  // matches expwinver of hModule                     // 匹配的expwinver of hModule

    DWORD         dwUserData;   // Reserved for random application data             // 给随机应用数据保留
    HDC           hdcOwn;                                                           // hdc句柄
    HRGN          hrgnClip;     // Clipping region for this window                  // 针对当前窗口的剪切区域
    int           iHungRedraw;  // Index into hung redraw table                     // hung redraw table的index
#ifdef FE_IME
    DWORD         hImc;         // Associated input context handle                  // 关联的输入法context 句柄
#endif

    WW;         // WOW-USER common fields. Defined in wowuserp.h                    // WOW-USER 通用fields包括一些样式的值
...
typedef struct tagDESKTOPINFO {
    ...
    PVOID                 pvDesktopBase;          // For handle validation      // 桌面对象基址
    PVOID                 pvDesktopLimit;         //                            // 桌面Limit
    struct tagWND         *spwnd;                 // Desktop window             // 桌面的顶层窗口
    DWORD                 fsHooks;                // Deskop global hooks        // 桌面全局钩子数量
    struct tagHOOK        *asphkStart[CWINHOOKS]; // List of hooks              // 桌面全局钩子链表
    struct tagWND         *spwndShell;            // Shell window               // Shell Window
    DWORD                 idShellProcess;         // Shell Process ID           // Shell Process ID
    struct tagWND         *spwndBkGnd;            // Shell background window    // Shell 背景窗口
    struct tagWND         *spwndTaskman;          // Task-Manager window        // 任务管理器窗口
    struct tagWND         *spwndProgman;          // Program-Manager window     // 程序管理器窗口
    int                   nShellHookPwnd;         //                            // ??
