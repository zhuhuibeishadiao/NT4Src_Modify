...
/* 这个函数异步地发送一个消息给一个窗口。前4个参数，hwnd, message, wParam, lParam 会传递给对应接受窗口的消息处理函数
   如果接受消息的窗口属于当前发送消息线程的队列，则窗口过程直接被调用。否则，我们会设置一个消息结构，等待合适的线程来接受消息，同时给他一个回调函数，用于处理结果*/
    ...
    BOOL fClientRequest)
{
     ...
     */
    /* 检测是否是承载的消息是否是DWORD，不允许一个应用程序携带一个指针和句柄 */
...
/* 该函数用来进行线程间消息的传递，如果参数ptiSender是空，则指明该函数是xxxSendNotifyMessage()调用过来的，同时要做相应的动作 */
...
fnINSTRINGThunk:
    ...
    *ppsms = psms;
    ...
    if (ptiSender != NULL) 
    {
        /* 如果Sender不为空，则将该消息同时插入到Sender的发送队列中 */
        /*
    ...
    else
    {
         ...
         * chance to free it.
         */
        /* 设置receive free标志，因为我们会直接返回，没有机会去释放消息 */
