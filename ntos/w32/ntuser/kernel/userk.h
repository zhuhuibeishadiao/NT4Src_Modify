 ...
 */
/* 以下是一个线程锁结构，该结构存在于栈上，链入一个线程信息结构的LIFO 链表， */
typedef struct _TL {
    struct _TL *next;
    PVOID pobj;
#ifdef DEBUG
...
typedef struct tagWINDOWSTATION {
    struct tagWINDOWSTATION *rpwinstaNext;                          // 窗口站链表指针
    struct tagDESKTOP *rpdeskList;                                  // 其下所有桌面的链表
    struct tagDESKTOP *rpdeskLogon;                                 // 已经登录的桌面链表
    struct tagDESKTOP *rpdeskDestroy;                               // 已经销毁的桌面链表
    PKEVENT           pEventDestroyDesktop;                         // 窗口销毁事件

    /*
     * Pointer to the currently active desktop for the window station.
     */
    struct tagDESKTOP    *rpdeskCurrent;                            // 当前激活的窗口
    struct tagWND        *spwndDesktopOwner;                        // 桌面拥有者
    struct tagWND        *spwndLogonNotify;                         // 什么什么登录通知
    struct tagTHREADINFO *ptiDesktop;                               // 当前桌面的线程信息
    DWORD                dwFlags;                                   // 标志
    struct tagKL         *spklList;
    LPWSTR               pwchDiacritic;                             // 不知道干嘛的
    WCHAR                awchDiacritic[5];                          // ...
    PKEVENT              pEventInputReady;                          // ......

    /*
     * Clipboard variables                                          // 剪切板相关的变量
     */
    PTHREADINFO          ptiClipLock;                               // 剪切板锁
    PWND                 spwndClipOpen;                             // 剪切板打开
    PWND                 spwndClipViewer;                           // 剪切板viewer
    PWND                 spwndClipOwner;                            // 剪切板owner
    struct tagCLIP       *pClipBase;                                // 剪切板消息头
    int                  cNumClipFormats;                           // 剪切板格式数量
    UINT                 iClipSerialNumber;                         // 剪切板序列号
    UINT                 fClipboardChanged : 1;                     // 剪切板change
    UINT                 fDrawingClipboard : 1;                     // 描绘剪切板

    /*
     * Global Atom table                                            // 全局原子信息表
     */
    PVOID                pGlobalAtomTable;                          // 原子信息表指针

    PKEVENT              pEventSwitchNotify;                        // 事件切换的通知
    LUID                 luidEndSession;                            // 结束会话的LUID
    LUID                 luidUser;                                  // USER的LUID
    PSID                 psidUser;                                  // USER的PSID
    PQ                   pqDesktop;                                 // 桌面
    HANDLE               hEventSwitchNotify;                        // 切换事件通知
...
//***************************************** begin: USER specific fields
    ...
    LIST_ENTRY      PtiLink;            // Link to other threads on desktop                                     // 当前桌面的线程链表
    PTL             ptl;                // Listhead for thread lock list                                        // 线程锁链表的头
    PTL             ptlOb;              // Listhead for kernel object thread lock list                          // 内核对象线程锁链表的头
    PTL             ptlPool;            // Listhead for temp pool usage                                         // 临时pool usage的链表头
    int             cEnterCount;

    struct tagPROCESSINFO *ppi;         // process info struct for this thread                                  // 当前线程的进程信息

    struct tagQ    *pq;                 // keyboard and mouse input queue                                       // 键盘和鼠标消息输入队列

    PKL             spklActive;         // active keyboard layout for this thread                               // 当前线程使用的激活的键盘布局
    MLIST           mlPost;             // posted message list.                                                 // POST的消息链表
    USHORT          fsChangeBitsRemoved;// Bits removed during PeekMessage                                      // 再PeekMessage过程中被移除的bits
    USHORT          cDeskClient;        // Ref count for CSRSS desktop                                          // CSRSS desktop 的引用计数

    PCLIENTTHREADINFO pcti;             // Info that must be visible from client                                // 客户端必须显示的信息，这些信息用来指明当前引用程序接收到了什么或者正在做什么，SetWakeBit中会设置该位
    CLIENTTHREADINFO  cti;              // Use this when no desktop is available                                // 当没有可用桌面时使用

    HANDLE          hEventQueueClient;                                                                          //
    PKEVENT         pEventQueueServer;                                                                          //
    
    PKEVENT        *apEvent;            // Wait array for xxxPollAndWaitForSingleObject                         // 对于xxxPollAndWaitFosSingleObject的等待数组

    PDESKTOP        rpdesk;                                                                                     // 桌面
    HDESK           hdesk;              // Desktop handle                                                       // 桌面句柄
    ACCESS_MASK     amdesk;             // Granted desktop access                                               // 桌面访问权限
    PDESKTOPINFO    pDeskInfo;          // Desktop info visible to client                                       // 对于client可显示的桌面信息
    PCLIENTINFO     pClientInfo;        // Client info stored in TEB                                            // 存储在TEB中的client info

    DWORD           TIF_flags;          // TIF_ flags go here.                                                  // TIF Flags

    PUNICODE_STRING pstrAppName;        // Application module name.                                             // 程序模块名

    struct tagSMS  *psmsSent;           // Most recent SMS this thread has sent                                 // 当前线程发送的大部分最近的消息
    struct tagSMS  *psmsCurrent;        // Received SMS this thread is currently processing                     // 当前线程收到的当前正在处理的消息
    struct tagSMS  *psmsReceiveList;    // SMSs to be processed                                                 // 即将被处理的消息，通过SendMessage发送来的消息会插入到这个队列中

    LONG            timeLast;           // Time, position, and ID of last message                               // 最后一条发来的消息时间
    POINT           ptLast;                                                                                     // ....位置
    DWORD           idLast;                                                                                     // ... ID
    ...
    union {
        PTDB            ptdb;           // Win16Task Schedule data for WOW thread                               // wow线程的win16task Schedula 数据
        PWINDOWSTATION  pwinsta;        // Window station for SYSTEM thread                                     // 系统线程的窗口站
        PDESKTOP        pdeskClient;    // Desktop for CSRSS thread                                             // CSRSS 线程的窗口
    };

    PSVR_INSTANCE_INFO psiiList;        // thread DDEML instance list                                           // 线程 DDEML 实例链表
    DWORD           dwExpWinVer;
    DWORD           dwCompatFlags;      // The Win 3.1 Compat flags                                             // Win 3.0 兼容标志

    UINT            cWindows;           // Number of windows owned by this thread                               // 当前线程所拥有的窗口数量
    UINT            cVisWindows;        // Number of visible windows on this thread                             // 当前线程可见的窗口数量

    struct tagQ    *pqAttach;           // calculation variabled used in                                        // 计算AttachThreadInput变量的使用情况
                                        // AttachThreadInput()

    int             iCursorLevel;       // keep track of each thread's level                                    // 记录没个线程的级别
    DWORD           fsReserveKeys;      // Keys that must be sent to the active                                 // 必须发送给激活控制台窗口的keys
                                        // active console window.
    struct tagTHREADINFO *ptiSibling;   // pointer to sibling thread info                                       // 指向 sibling thread info

    PMOVESIZEDATA   pmsd;

    DWORD           fsHooks;                // WHF_ Flags for which hooks are installed                          // WHF 标志 哪些钩子被安装了
    PHOOK           asphkStart[CWINHOOKS];  // Hooks registered for this thread                                  // 当前线程注册的钩子
    PHOOK           sphkCurrent;            // Hook this thread is currently processing                          // 当前正在处理的钩子

    PSBTRACK        pSBTrack;

#ifdef FE_IME
    PWND            spwndDefaultIme;    // Default IME Window for this thread                                    // 当前线程默认的IME
    PIMC            spDefaultImc;       // Default input context for this thread                                 // 当前线程默认的输入上下文
    HKL             hklPrev;            // Previous active keyboard layout                                       // 上一次激活的键盘布局
#endif
} THREADINFO;
...
typedef struct tagBWL {
    ...
    HWND          *phwndNext;       // 一般为rghWnd数组的头，也就是rghWnd地址
    HWND          *phwndMax;        // 一般为rghWnd数组的最后一个元素
    PTHREADINFO   ptiOwner;         // 所属线程
    HWND          rghwnd[1];        // PWND*n个大小的数组
} BWL, *PBWL;
