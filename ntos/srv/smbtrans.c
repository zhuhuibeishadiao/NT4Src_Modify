...

{
    ...
    if ( transaction->OutParameters == NULL ) 
    {
        ...
        transaction->OutParameters = (PCHAR)(transaction->OutSetup + transaction->MaxSetupCount);
    ...
    if ( transaction->OutData == NULL ) 
    {
        ...
        transaction->OutData = transaction->OutParameters + transaction->MaxParameterCount;
...

{
    ...
    if ( ResultStatus == SmbTransStatusErrorWithoutData ) {
        ...
        IF_SMB_DEBUG(TRANSACTION1) {
            SrvPrint1( "Error response. Closing transaction 0x%lx\n",
                        transaction );
        }
    ...
    if ( (transaction->Flags & SMB_TRANSACTION_NO_RESPONSE) &&
        ...
        IF_SMB_DEBUG(TRANSACTION1) {
            SrvPrint1( "No response.  Closing transaction 0x%lx\n",
                        transaction );
        }
    ...
    if ( transaction->OutputBufferCopied ) {
        ...
        paramOffset = paramPtr - (PCHAR)header;  // offset from start of header
        ...
        if ( (paramOffset + paramLength) > maxSize ) {
            ...
            dataOffset = dataPtr - (PCHAR)header;   // offset from start of header
...

{
    ...
    IF_DEBUG(TRACE2) {
        SrvPrint2( "  connection 0x%lx, endpoint 0x%lx\n",
    ...
    if ( WorkContext->Irp->Cancel ||
        ...
        IF_DEBUG(ERRORS) {
            SrvPrint1( "I/O error. Closing transaction 0x%lx\n", transaction );
        }
        SrvCloseTransaction( transaction );
        SrvDereferenceTransaction( transaction );
    ...
    IF_SMB_DEBUG(TRANSACTION1) {
        SrvPrint2( "Continuing transaction response; block 0x%lx, name %wZ\n",
                    transaction, &transaction->TransactionName );
        SrvPrint3( "Connection 0x%lx, session 0x%lx, tree connect 0x%lx\n",
...

{
    ...
    if ( request->WordCount != (UCHAR)(14 + request->SetupCount) ) {
    ...
    if ( pipeRequest && !remoteApiRequest ) {
        ...
        if ( remoteApiRequest ) {
            ...
            if ( SrvXsPortMemoryHeap == NULL ) {
                ...
                IF_DEBUG(ERRORS) {
                    SrvPrint0( "SrvSmbTransaction: The XACTSRV service is "
                              "not started.\n" );
                }
    ...
    if ( singleBufferTransaction ) {
        ...
        return ExecuteTransaction( WorkContext );
...

{
    ...
    /* 查找匹配当前secondary request的事务块。一个事务中的所有消息的TID，PID，UID和MID都一样。
      如果找到一个匹配的，增加引用，返回地址*/
    connection = WorkContext->Connection;
...

{
    ...
    /* 校验WordCount是否正确 */
    if ( request->WordCount != (UCHAR)(19 + request->SetupCount) ) 
    {
    ...
    //
    // BUGBUG we will later quad-word align input buffer for OFS query
    ...
    // completely in the primary buffer.
    //
    // !!! We could allow secondary requests to start by allocating a
    //     separate buffer for the interim response, sending the
    //     response, then copying the data.
...

{
    ...
    if ( fullName.Buffer == NULL ) {
        ...
        IF_DEBUG(ERRORS) {
            SrvPrint0( "MailslotTransaction: Unable to allocate heap for "
                        "full path name\n" );
        }
